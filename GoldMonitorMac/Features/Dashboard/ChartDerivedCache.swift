...
final class ChartDerivedCache: ObservableObject {
    ...
    /// Includes the trailing bar's TS/close/volume: the block lifecycle,
    /// the VP ranking and the drawn profile all depend on the live bar,
    /// which rewrites in place without changing `count`.
    ...
    private let rankedOBSlot = Slot<RankedOBSig, RankedOrderBlocks.Output>(.empty)
    ...
    func rankedOrderBlocks(
        ...
        let sig = RankedOBSig(
            ...
            dispMult: dispMult, atrLen: atrLen, zoneSrc: zoneSrc, mitBy: mitBy,
            useBOS: useBOS, bosLen: bosLen, maxOBs: maxOBs, removeMit: removeMit,
            useVP: useVP, vpLookback: vpLookback, vpRows: vpRows, useIchi: useIchi,
            tenkan: tenkan, kijun: kijun, senkouB: senkouB, ichiDisp: ichiDisp
        )
    ...
    struct OverlayData {
        ...
        /// The Ichimoku overlay the Ranked-OB indicator draws (only when
        /// its "Show Ichimoku" toggle is on) — folded into the yDomain so
        /// the cloud doesn't clip. `.empty` when the indicator is off.
        let rankedOBIchimoku: Ichimoku.Output
    ...
    func overlayExtremes(_ data: OverlayData) -> (lo: Double, hi: Double) {
        let sig = OverlayExtremesSig(
            ...
            rankedOBIchiLineCount: data.rankedOBIchimoku.kijun.count,
        ...
        return resolve(overlayExtremesSlot, signature: sig) {
            ...
            // Ranked-OB Ichimoku overlay — scan its spans so the cloud
            // stays inside the yDomain (same treatment as the standalone
            // Ichimoku indicator below).
            let robIchi = data.rankedOBIchimoku
            for p in robIchi.senkouA { if p.value < lo { lo = p.value }; if p.value > hi { hi = p.value } }
            for p in robIchi.senkouB { if p.value < lo { lo = p.value }; if p.value > hi { hi = p.value } }
            for p in robIchi.tenkan { if p.value < lo { lo = p.value }; if p.value > hi { hi = p.value } }
            for p in robIchi.kijun { if p.value < lo { lo = p.value }; if p.value > hi { hi = p.value } }
