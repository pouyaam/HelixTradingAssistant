...
| `AI/` | `AIEngine` protocol, `ClaudeEngine` (claude CLI shell-out + stream-json parser), `CodexEngine` (Coming Soon stub), `PromptBuilder` (per-kind system prompts + structured-data parsers), `AnalysisStore` (per-pair sessions + history persistence) |
...
2. **AIEngine** (`AI/AIEngine.swift`) — protocol with `run(system:user:)`
   ...
   - **ClaudeEngine** spawns `claude --print --output-format stream-json
     ...
     reuses the Claude Code CLI's own session.
...
- **Not for the App Store** — uses unsandboxed process spawning,
  open ATS, claude CLI shell-out. Distribution path is "build it
